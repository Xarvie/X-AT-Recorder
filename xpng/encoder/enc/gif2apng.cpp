...
unsigned char * op_zbufg2p;
z_stream        op_zstreamg2p;
z_stream        fin_zstreamg2p;

unsigned int    next_seq_numg2p;
unsigned char   png_signg2p[8]      = {137,  80,  78,  71,  13,  10,  26,  10};
unsigned char   png_Softwareg2p[24] = { 83, 111, 102, 116, 119,  97, 114, 101, '\0',
...
void write_chunkg2p(FILE * f, const char * name, unsigned char * data, unsigned int length)
{
  ...
  {
    unsigned int seq = swap32(next_seq_numg2p++);
    fwrite(&seq, 1, 4, f);
...
void deflate_rect_fin(int deflate_method, int iter, unsigned char * zbuf, int * zsize, int bpp, int stride, unsigned char * rows, int zbuf_size, int n)
{
  ...
  {
    fin_zstreamg2p.next_out = zbuf;
    fin_zstreamg2p.avail_out = zbuf_size;
    fin_zstreamg2p.next_in = rows;
    fin_zstreamg2p.avail_in = op[n].h*(rowbytes + 1);
    deflate(&fin_zstreamg2p, Z_FINISH);
    *zsize = fin_zstreamg2p.total_out;
    deflateReset(&fin_zstreamg2p);
  }
}
...
void deflate_rect_op(unsigned char * p, int x, int y, int w, int h, int bpp, int stride, unsigned char * row_buf, int zbuf_size, int n)
{
  ...
  op_zstreamg2p.data_type = Z_BINARY;
  op_zstreamg2p.next_out = op_zbufg2p;
  op_zstreamg2p.avail_out = zbuf_size;
  for (j=0; j<h; j++)
  {
    memcpy(row_buf+1, row, w*bpp);
    op_zstreamg2p.next_in = row_buf;
    op_zstreamg2p.avail_in = w*bpp + 1;
    deflate(&op_zstreamg2p, Z_NO_FLUSH);
    row += stride;
  }
  deflate(&op_zstreamg2p, Z_FINISH);
  op[n].p = p;
  op[n].size = op_zstreamg2p.total_out;
  op[n].x = x;
  op[n].y = y;
  op[n].w = w;
  op[n].h = h;
  op[n].valid = 1;
  deflateReset(&op_zstreamg2p);
}
...
void write_IDATsg2p(FILE * f, unsigned int frame, unsigned char * data, unsigned int length, unsigned int idat_size)
{
	...
	while (length > 0)
	{
		...
		if (frame == 0)
			write_chunkg2p(f, "IDAT", data, ds);
		else
			write_chunkg2p(f, "fdAT", data, ds + 4);

		data += ds;
		length -= ds;
	}
}
int gif2apng_main(int argc, char** argv)
{
  int              i, j, k, n, start;
  ...
  if ((f1 = fopen(szIn, "rb")) != 0)
  {
    ...
    if ((f2 = fopen(szOut, "wb")) != 0)
    {
      ...
      fwrite(png_signg2p, 1, 8, f2);

      write_chunkg2p(f2, "IHDR", (unsigned char *)(&ihdr), 13);

      if (frames > 1)
        write_chunkg2p(f2, "acTL", (unsigned char *)(&actl), 8);

      if (plte_size > 0 && coltype == 3)
        write_chunkg2p(f2, "PLTE", (unsigned char *)(&plte), plte_size*3);

      if (trns_size > 0)
        write_chunkg2p(f2, "tRNS", trns, trns_size);

      idat_size = (rowbytes + 1) * h;
      zbuf_size = idat_size + ((idat_size + 7) >> 3) + ((idat_size + 63) >> 6) + 11;

      op_zstreamg2p.data_type = Z_BINARY;
      op_zstreamg2p.zalloc = Z_NULL;
      op_zstreamg2p.zfree = Z_NULL;
      op_zstreamg2p.opaque = Z_NULL;
      deflateInit2(&op_zstreamg2p, Z_BEST_SPEED, 8, 15, 8, Z_DEFAULT_STRATEGY);

      fin_zstreamg2p.data_type = Z_BINARY;
      fin_zstreamg2p.zalloc = Z_NULL;
      fin_zstreamg2p.zfree = Z_NULL;
      fin_zstreamg2p.opaque = Z_NULL;
      deflateInit2(&fin_zstreamg2p, Z_BEST_COMPRESSION, 8, 15, 8, Z_DEFAULT_STRATEGY);

      op_zbufg2p = (unsigned char *)malloc(zbuf_size);
      if (op_zbufg2p == NULL)
        return 1;
      ...
      while ( !feof(f1) )
      {
        ...
        if (id == 0x2C)
        {
          ...
          else
          {
            ...
            fctl.mSeq       = swap32(next_seq_numg2p++);
            ...
            fctl.mDisposeOp = dop;
            fctl.mBlendOp   = bop;
            write_chunkg2p(f2, "fcTL", (unsigned char *)(&fctl), 26);

            write_IDATsg2p(f2, n-1, zbuf, zsize, idat_size);
      ...
      write_IDATsg2p(f2, frames-1, zbuf, zsize, idat_size);

      free(row_buf);
      free(rows);
      free(zbuf);

      deflateEnd(&fin_zstreamg2p);
      deflateEnd(&op_zstreamg2p);
      free(op_zbufg2p);

      write_chunkg2p(f2, "tEXt", png_Softwareg2p, 24);
      write_chunkg2p(f2, "IEND", 0, 0);
      fclose(f2);
      printf("%d frames.\n", n);
    }
    else
